...
function VehicleSelection() {
  ...
  const [bookingDetails, setBookingDetails] = useState({
    pickupAddress: "", // Matches `pickupAddress` in `prepareBookingPayload`
    destinationAddress: "", // Matches `destinationAddress`
    pickupDateTime: "", // Matches `pickupDateTime`
  });
  ...
  useEffect(() => {
    ...
    if (details) {
      setBookingDetails({
        pickupAddress: details.from || "", // Map `from` to `pickupAddress`
        destinationAddress: details.to || "", // Map `to` to `destinationAddress`
        pickupDateTime: details.date
          ? `${details.date} ${details.time}` // Combine date and time
          : "",
      });
    }
  }, []);
  ...
  const handleNext = () => {
    if (selectedVehicle) {
      const updatedBookingDetails = {
        ...bookingDetails,
        vehicleName: selectedVehicle.name, // Add vehicle details
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {vehicle.passengers} Passengers, {vehicle.bags} Bags,{" "}
